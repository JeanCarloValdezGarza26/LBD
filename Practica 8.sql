CREATE DATABASE RestauranteLSTI10
USE RestauranteLSTI10
...
(Id_Restaurante CHAR(3) PRIMARY KEY NOT NULL,  
   ...
   Sucursal_Ubicacion varchar(30) NOT NULL,
	...
	--TRIGGER--
	CREATE TRIGGER Ejemplo_Factura
	ON Mesa FOR INSERT AS
	BEGIN 
	 DECLARE @Mesa AS INT
	 DECLARE @Cantidad AS INT

	 SET @Mesa =(SELECT id_Mesa FROM INSERTED)
	 SET @Cantidad =(SELECT CantMax_Comensal FROM INSERTED)

	 UPDATE Factura
	 SET N_Factura = N_Factura + @Cantidad
	 WHERE Id_Mesa LIKE @Mesa

	 END

	 --TRIGGER2--
	CREATE TRIGGER Ejemplo_Platillos
	ON Platillos FOR INSERT AS
	BEGIN 
	 DECLARE @Platillos AS INT
	 DECLARE @Precio_Platillo AS INT

	 SET @Platillos =(SELECT Porcion_Platillo FROM INSERTED)
	 SET @Precio_Platillo =(SELECT Precio_Platillo FROM INSERTED)

	 UPDATE Bebidas
	 SET Precio_Bebida = Precio_Bebida + @Precio_Platillo
...
CREATE VIEW V_Restaurante
AS SELECT Nom_Restaurante, Telefono, Sucursal_Ubicacion
FROM Restaurante;

SELECT * FROM V_Restaurante;

--PROCEDIMIENTO ALMACENADO 1--
CREATE PROCEDURE SP_Prueba
AS 
SELECT * FROM Restaurante
WHERE Id_Restaurante = 'R01'
...
(Id_Mesa CHAR(3) PRIMARY KEY NOT NULL,  
   ...
   Seccion_Restaurante varchar(15) NOT NULL,
	...
	CREATE VIEW V_Mesa
	AS SELECT CantMax_Comensal, Seccion_Restaurante
	FROM Mesa;
...
(Usuario_Cliente varchar (30) PRIMARY KEY NOT NULL, 
   ...
   Notas_Cliente varchar (30) NULL,
	...
	AS SELECT Usuario_Cliente, Direccion_Cliente, Notas_Cliente
	FROM Cliente;

	SELECT * FROM V_Cliente;

	SELECT * FROM Cliente
	WHERE Id_Mesa IN (SELECT Id_Mesa FROM Cliente)
...
(Tipo_P varchar (12) PRIMARY KEY NOT NULL,  
   ...
   SELECT * FROM Tipo_Personal
   	...
   	CREATE VIEW V_TPersonal
	AS SELECT Tipo_P
	FROM Tipo_Personal;

	SELECT * FROM V_TPersonal;

	--PROCEDIMIENTO ALMACENADO 4--
	CREATE PROCEDURE SP_Prueba4
	AS 
	SELECT * FROM Tipo_Personal
	WHERE Tipo_P = 'Corredor'
...
(Id_Turno CHAR(3) PRIMARY KEY NOT NULL,  
   Desc_Turno varchar(30) NOT NULL,
	...
	CREATE VIEW V_Turno
	AS SELECT Desc_Turno
	FROM Turno;

	SELECT * FROM V_Turno;

	--PROCEDIMIENTO ALMACENADO 5--
	CREATE PROCEDURE SP_Prueba5
	AS 
	SELECT * FROM Turno
	WHERE Id_Turno = 'T09'
...
(Id_Personal CHAR(3) PRIMARY KEY NOT NULL,  
   ...
   Id_Turno CHAR(3) NOT NULL FOREIGN KEY (Id_Turno) REFERENCES Turno(Id_Turno),
	...
	AS SELECT Nom_Personal, Direccion_Personal
	FROM Personal;

	SELECT * FROM V_Personal;

	--PROCEDIMIENTO ALMACENADO 6--
	CREATE PROCEDURE SP_Prueba6
	AS 
	SELECT * FROM Personal
	WHERE Id_Personal = 'P02'
...
(Id_Platillo CHAR(4) PRIMARY KEY NOT NULL,  
   ...
   Id_Personal CHAR(3) NOT NULL FOREIGN KEY (Id_Personal) REFERENCES Personal(Id_Personal),
	...
	AS SELECT Nom_Platillo, Porcion_Platillo, Precio_Platillo
	FROM Platillos;

	SELECT * FROM V_Platillo;

	--PROCEDIMIENTO ALMACENADO 7--
	CREATE PROCEDURE SP_Prueba7
	AS 
	SELECT * FROM Platillos
	WHERE Id_Platillo = 'PL06'
...
(Id_Bebida CHAR(3) PRIMARY KEY NOT NULL,  
   ...
   Id_Personal CHAR(3) NOT NULL FOREIGN KEY (Id_Personal) REFERENCES Personal(Id_Personal),
	...
	CREATE VIEW V_Bebida
	AS SELECT Nom_Bebida, Des_Bebida, Precio_Bebida
	FROM Bebidas;

	SELECT * FROM V_Bebida;

	--PROCEDIMIENTO ALMACENADO 8--
	CREATE PROCEDURE SP_Prueba8
	AS 
	SELECT * FROM Bebidas
	WHERE Id_Bebida = 'B04'
...
(N_Factura int PRIMARY KEY NOT NULL,  
	...
	AS SELECT Id_Restaurante, Id_Mesa, Id_Bebida
	FROM Factura;

	SELECT * FROM V_Factura;

	--PROCEDIMIENTO ALMACENADO 9--
	CREATE PROCEDURE SP_Prueba9
	AS 
	SELECT * FROM Factura
	WHERE N_Factura = '01'
...
(Id_Pedido int PRIMARY KEY NOT NULL,  
   ...
   SELECT * FROM Comanda
	...
	CREATE VIEW V_Comanda
	AS SELECT Id_Pedido, BebidaP, Ruta_Pedido
	FROM Comanda;

	SELECT * FROM V_Comanda;

	--PROCEDIMIENTO ALMACENADO 10--
	CREATE PROCEDURE SP_PruebaF10
	AS 
	SELECT * FROM Comanda
	WHERE Ruta_Pedido = 'Esquina'

	EXEC SP_PruebaF10

	--CREAR FUNCION--
	--CALCULAR PROMEDIO DE LAS VENTAS EJEMPLO--
	CREATE FUNCTION calcularventasprom
	...
	SELECT dbo.calcularventasprom(12,13) as promedio 
	GO
